...
function buildRecommendations(limitCount) {
  ...
  const userCategoryAffinity = database.events
    .aggregate([
      ...
      { $match: { resolved_category_id: { $ne: null } } },
      {
        $group: {
          _id: {
      ...
      { $match: { category_affinity_score: { $gt: 0 } } },
      {
        $project: {
          _id: 0,
  ...
  const productPopularity = database.events
    .aggregate([
      ...
      { $match: { product_popularity_score: { $gt: 0 } } },
      {
        $project: {
          _id: 0,
          product_id: "$_id",
...
function buildSearchQuery(topRecommendations) {
  const normalizedTerms = [];
  const seen = new Set();

  for (const row of topRecommendations) {
    const normalized = String(row.category_code || "")
      .replace(/[._-]+/g, " ")
      .trim();
    if (!normalized) {
      continue;
...
const topRecommendations = buildRecommendations(5);
const queryText = buildSearchQuery(topRecommendations.recommendations);
const searchResults =
  queryText === ""
    ? []
    : database.products
        ...
        .toArray()
        .map((row) => ({
          target_user_id: topRecommendations.targetUserId,
...
printSection("Q3 / Full-text search results from recommended categories", searchResults);
